...
# VAE
class VAEFitter(TorchFitterBase):
    ...
    def fit(self,
        ...
        for e in range(n_epochs):
            ...
            history['train_KLD_loss'] = train_loss['KLD'].avg
            history['train_reconstruction_loss'] = train_loss['recon'].avg
            ...
            if val_loader is not None:
                ...
                val_loss, calculated_metrics = self.validation(val_loader,
                                                               metrics=metrics,
                                                               verbose_steps=verbose_steps)
                history['val_KLD_loss'] = val_loss['KLD'].avg
                history['val_reconstruction_loss'] = val_loss['recon'].avg
                history['val_summary_loss'] = val_loss['summary'].avg

                # Write log
                metric_log = ' - ' + ' - '.join([f'{fname}: {value}' for value, fname in calculated_metrics]) if calculated_metrics else ''
                self.log(f"\r[RESULT] {(time.time() - t):.2f}s | " +
                         f"train KLD loss: {train_loss['KLD'].avg:.5f} | " +
                         f"train reconstruction loss: {train_loss['recon'].avg:.5f} | " +
                         f"train summary loss: {train_loss['summary'].avg:.5f} | " +
                         f"val KLD loss: {val_loss['KLD'].avg:.5f} | " +
                         f"val reconstruction loss: {val_loss['recon'].avg:.5f} | " +
            ...
            if callbacks is not None:
                if not isinstance(callbacks, list):
                    callbacks = [callbacks]
                for c in callbacks:
                    c(history, val_loader, self.model, self.device)
    ...
    def train_one_epoch(self, train_loader, verbose_steps=0):
        ...
        for step, data in enumerate(train_loader):
            if self.verbose & (verbose_steps > 0):
                if step % verbose_steps == 0:
                    print(
                        f'\rTrain Step {step}/{len(train_loader)} | ' +
                        f'KLD_loss: {KLD_loss.avg:.5f} | ' +
                        f'reconstruction_loss: {recon_loss.avg:.5f} | ' +
                        f'summary_loss: {summary_loss.avg:.5f} | ' +
                        f'time: {(time.time() - t):.2f} secs | ' +
                        f'ETA: {(len(train_loader)-step)*(time.time() - t)/(step+1):.2f}', end=''
                    )
            ...
            # Run one batch
            loss = self.train_one_batch(x)
            
            KLD_loss.update(loss['KLD'].detach().item(), batch_size)
            recon_loss.update(loss['recon'].detach().item(), batch_size)
        ...
        return {'KLD':KLD_loss, 'recon':recon_loss, 'summary':summary_loss}
    ...
    def train_one_batch(self, x):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            loss = self.loss_function(output, x, self.epoch+1)
    ...
    # Adapt validation step to PyTorch SSIM metric
    def validation(self, val_loader, metrics=None, verbose_steps=0):
        """
        Validates a model
        Parameters
            ...
            if self.verbose & (verbose_steps > 0):
                if step % verbose_steps == 0:
                    print(
                        f'\rVal Step {step}/{len(val_loader)} | ' +
                        f'KLD_loss: {KLD_loss.avg:.5f} | ' +
                        f'recon_loss: {recon_loss.avg:.5f} | ' +
                        f'summary_loss: {summary_loss.avg:.5f} | ' +
                        f'time: {(time.time() - t):.2f} secs |' +
                        f'ETA: {(len(val_loader)-step)*(time.time() - t)/(step+1):.2f}', end=''
                    )
            with torch.no_grad():  # no gradient update
                batch, y = self.unpack(data)
                ...
                if metrics is not None:
                    for f in metrics:
                        batch_metrics[f.__name__].append(f.forward(recon_batch, batch))
        ...
        return {'KLD':KLD_loss, 'recon':recon_loss, 'summary':summary_loss}, calculated_metrics
